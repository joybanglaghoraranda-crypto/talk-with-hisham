...
export default function MyMessages() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('inbox.sent_toast', locale));
      setNewMessage('');
      fetchMessages();
      ...
      try {
        await sendEmail({
          ...
          html: `
            <h3>{t('inbox.email_new', locale)}</h3>
            <p><strong>{t('inbox.email_sender', locale)}</strong> ${user.email}</p>
            <p><strong>{t('inbox.email_msg', locale)}</strong></p>
            <div style="padding: 12px; background-color: #f3f4f6; border-left: 4px solid #6366f1; border-radius: 4px; font-style: italic;">
              "${messageContent}"
            </div>
            <p>{t('inbox.email_login', locale)} <a href="${window.location.origin}/admin">{t('inbox.email_admin', locale)}</a> {t('inbox.email_reply', locale)}</p>
          `,
        });
      } catch (emailErr) {
  ...
  if (!user) return (<div className="flex flex-col items-center justify-center min-h-[50vh] text-center"><Mail className="text-white/10 mb-4" size={48} /><h2 className="text-xl font-heading font-bold text-white mb-2">{t('inbox.signin_title', locale)}</h2><p className="text-white/35 text-sm">{t('inbox.signin_desc', locale)}</p></div>);

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto py-6">
      <div className="mb-6"><h1 className="text-2xl font-heading font-bold text-white tracking-tight flex items-center gap-2"><Lock size={20} className="text-brand-400" /> {t('inbox.private', locale)}</h1><p className="text-white/30 text-sm">{t('inbox.desc', locale)}</p></div>
      <form onSubmit={handleSend} className="glass-card p-5 mb-6">
        <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={t("inbox.write_ph", locale)} rows={3} className="w-full bg-transparent outline-none text-sm text-white placeholder:text-white/20 resize-none" />
        <div className="flex justify-end mt-2">
          <button type="submit" disabled={sending || !newMessage.trim()} className="flex items-center gap-2 bg-gradient-to-r from-brand-500 to-accent-500 px-5 py-2 rounded-lg text-sm font-bold text-white disabled:opacity-30 transition-all hover:scale-105 active:scale-95">
            {sending ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />} {t('inbox.send', locale)}
          </button>
        </div>
      </form>
      {loading ? <div className="space-y-3">{[1,2,3].map(i=><div key={i} className="glass-card p-5 h-24 skeleton"/>)}</div>
      : messages.length === 0 ? <div className="glass-card py-16 text-center"><MessageSquare className="mx-auto text-white/10 mb-3" size={36}/><p className="text-white/20 text-sm">{t('inbox.empty_alt', locale)}</p></div>
      : <div className="space-y-3">{messages.map(msg=>(
        <motion.div key={msg.id} layout className="glass-card glass-card-hover p-5 cursor-pointer" onClick={()=>setSelectedMsg(selectedMsg?.id===msg.id?null:msg)}>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {msg.admin_reply?<span className="text-[10px] text-green-400/60 flex items-center gap-1"><CheckCircle2 size={10}/>{t('inbox.replied', locale)}</span>:<span className="text-[10px] text-brand-400/40">{t('inbox.awaiting', locale)}</span>}
          </div>
          {selectedMsg?.id===msg.id&&msg.admin_reply&&(
            <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} className="mt-3 pt-3 border-t border-white/5">
              <p className="text-[10px] text-green-400/60 uppercase tracking-widest font-semibold mb-1">{t('inbox.reply', locale)}</p>
