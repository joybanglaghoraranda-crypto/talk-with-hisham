...
export async function POST(request: Request) {
  try {
    ...
    // In production, we MUST use QStash for queueing and security controls.
    if (!isDev && !qstashClient) {
      console.error('CRITICAL: QStash client is not configured in production environment. Refusing to send email directly.');
      return NextResponse.json(
        { error: 'Internal server error: Email service configuration error' },
        { status: 500 }
      );
    }

    // Only allow bypassing QStash in development mode
