...
export async function POST(request: Request) {
  try {
    // SECURITY FIX: Authenticate user before allowing email to be sent
    ...
    // SECURITY FIX: Restrict the recipient scope (non-admins can only send to the admin)
    if (!isAdmin && to !== adminEmail) {
      return NextResponse.json({ error: 'Forbidden: Non-admins can only send emails to the administrator' }, { status: 403 });
    }
